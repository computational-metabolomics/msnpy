...
class ConvertTestCase(unittest.TestCase):
    ...
    def test_tree2peaklist(self):
        ...
        merged_original = load_peaklists_from_hdf5(to_test_data('test_merged_pls.hdf5'))
        for i in range(0, len(merged_original)):
            self.assertTrue(np.array_equal(merged_pls[i].peaks, merged_original[i].peaks))

        ms1_precursor_original = load_peaklists_from_hdf5(to_test_data('test_ms1_precursors_pl.hdf5'))
        for i in range(0, len(ms1_precursor_original)):
            self.assertTrue(np.array_equal(ms1_precursor_pl[i].peaks, ms1_precursor_original[i].peaks))

    def test_peaklist2msp_non_merged(self):
        test_out = os.path.join(tempfile.mkdtemp(), 'test_non_merged.msp')
        peaklist2msp(load_peaklists_from_hdf5(to_test_data('test_non_merged_pls.hdf5')),
                     test_out,
                     msp_type='massbank',
                     polarity='positive')
        self.assertListEqual(
            list(io.open(test_out)),
            list(io.open(to_test_data('test_non_merged.msp'))))

    def test_peaklist2msp_merged(self):

        #test_out = to_test_data('test_merged.msp')
        test_out = os.path.join(tempfile.mkdtemp(), 'test_merged.msp')
        peaklist2msp(load_peaklists_from_hdf5(to_test_data('test_merged_pls.hdf5')),
                     test_out,
                     msp_type='massbank',
                     polarity='positive')
        self.assertListEqual(
            list(io.open(test_out)),
            list(io.open(to_test_data('test_merged.msp'))))

    def test_peaklist2msp_ms1_precursors(self):
        #test_out = to_test_data('test_ms1_precursors.msp')
        test_out = os.path.join(tempfile.mkdtemp(), 'test_ms1_precursors.msp')
        peaklist2msp(load_peaklists_from_hdf5(to_test_data('test_ms1_precursors_pl.hdf5')),
                     test_out,
                     msp_type='massbank',
                     polarity='positive',
                     include_ms1=True)
        self.assertListEqual(
            list(io.open(test_out)),
            list(io.open(to_test_data('test_ms1_precursors.msp'))))